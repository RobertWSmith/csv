...
csvreader csvreader_init(csvdialect  dialect,
                         const char *filepath) {
  ZF_LOGI("`csvreader_init` called dialect: %p, filepath: %s", dialect, filepath);
...
csvreturn csvreader_next_record(csvreader       reader,
  ...
  while (true) {
    ...
    if (reader->parser_state == START_RECORD) {
      ZF_LOGD(
        "Parser state indicates the record boundry has been encountered, breaking out of loop.");
      break;
    }
  }
...
csvfilereader csvfilereader_init(void) {
  ...
  ZF_LOGD("`csvfilereader` successfully allocated atL %p", fr);
  return fr;
}
...
inline void parse_in_quoted_field(csvreader                reader,
  ...
  if (value == '\0') { /* no-op */ }
    ...
    reader->parser_state = ESCAPE_IN_QUOTED_FIELD;
           ...
           (QUOTE_STYLE_NONE != csvdialect_get_quotestyle(reader->dialect))) {
    if (csvdialect_get_doublequote(reader->dialect)) reader->parser_state =
        QUOTE_IN_QUOTED_FIELD;
    else reader->parser_state = IN_FIELD;
  }
  else {
    (*reader->appendchar)(reader->streamdata, value);
  }
